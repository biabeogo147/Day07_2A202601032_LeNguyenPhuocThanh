...
class EmbeddingStore:
                ...
                documents.append(
                    {
                        ...
                        "score": float(results["distances"][i]) if results["distances"] else 0.0,
                    }
                )
            return documents
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        if self._use_chroma and self._collection is not None:
            query_embedding = self._embedding_fn(query)
            filter_results = self._collection.query(
            ...
            for i in range(len(filter_results["ids"])):
                documents.append(
                    {
                        "id": filter_results["ids"][i],
                        "content": filter_results["documents"][i],
                        "metadata": filter_results["metadatas"][i],
                        "score": float(filter_results["distances"][i]) if filter_results["distances"] else 0.0,
                    }
                )
            return documents
        ...
        filtered = [
            ...
            and all(record["metadata"].get(k) == v for k, v in metadata_filter.items())
        ]
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        """
        if self._use_chroma and self._collection is not None:
            existing_ids = [str(_id) for _id in self._collection.get()['ids']]
